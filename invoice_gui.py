...
def process_invoices(base_path, buyer_keyword, output_path, log_callback):
    ...
    # 调用处理函数
    result = extractor.process_invoices(base_path, buyer_keyword, output_path)
    return result
...
class InvoiceGUI:
    ...
    def setup_ui(self):
        ...
        tk.Button(config_frame, text="浏览...", command=self.browse_dir).grid(row=0, column=2)
        ...
        tk.Button(config_frame, text="浏览...", command=self.browse_output).grid(row=2, column=2)
        ...
        self.install_btn = tk.Button(btn_frame, text="安装依赖", bg="#f39c12", fg="white",
                                    command=self.install_deps, font=("Arial", 10))
        ...
        self.run_btn = tk.Button(btn_frame, text="开始识别", bg="#27ae60", fg="white",
                                 command=self.run_extractor, font=("Arial", 11, "bold"),
                                 width=20)
